...
class GetDataSet(object):
    def __init__(self, dataSetName, isIID,style):
        ...
        if self.name == 'transformer':
            if self.style==0:
                self.shuffle_mnistDataSetConstruct(isIID)
            else:
    ...
    def mnistDataSetConstruct(self, isIID):
        ...
        # data_dir = r'../data/MNIST/'
        data_dir = r'../data/'
        train_traj_path=os.path.join(data_dir, 'normal_feas2304')
        # train_images_path = os.path.join(data_dir, 'train-images-idx3-ubyte.gz')
        # train_labels_path = os.path.join(data_dir, 'train-labels-idx1-ubyte.gz')
        # test_images_path = os.path.join(data_dir, 't10k-images-idx3-ubyte.gz')
        # test_labels_path = os.path.join(data_dir, 't10k-labels-idx1-ubyte.gz')
        #train_images = extract_images(train_images_path)
        input_datas = pickle.load(open(train_traj_path, 'rb'))
        input_datas = input_datas[:len(input_datas) ]
        #tdate = pickle.load(open('tdate', 'rb'))
        # t_block=[]
        # for i in range(len(input_datas)):
        #     t_block.append([input_datas[i],tdate[i]])
        # train_labels = extract_labels(train_labels_path)
        # test_images = extract_images(test_images_path)
        # test_labels = extract_labels(test_labels_path)
        dataset_len = len(input_datas)
        train_proportion = 0.6
        val_proportion = 0.2
        train_seq = input_datas[:int(dataset_len * train_proportion)]
        val_seq = input_datas[
                  int(dataset_len * train_proportion):int(dataset_len * (train_proportion + val_proportion))]
        test_seq = input_datas[int(dataset_len * (train_proportion + val_proportion)):]
        #assert train_seq.shape[0] == train_traj.shape[0]

        # assert train_images.shape[0] == train_labels.shape[0]
        # assert test_images.shape[0] == test_labels.shape[0]
        #
        self.train_data=train_seq
        self.train_data_size=len(train_seq)
        self.val_data = val_seq
        self.val_data_size = len(val_seq)
        self.test_data = test_seq
        self.test_data_size = len(test_seq)
        #self.train_data_size = len(train_seq)
        ...
        if isIID:
            ...
            self.train_data = train_seq[order]
            #self.train_label = train_seq[order]
        #else:
    ...
    def shuffle_mnistDataSetConstruct(self, isIID):
        #data_dir = r'.\data\MNIST'
        # data_dir = r'../data/MNIST/'
        data_dir = r'../data/'
        train_traj_path=os.path.join(data_dir, 'normal_feas9')
        # train_images_path = os.path.join(data_dir, 'train-images-idx3-ubyte.gz')
        # train_labels_path = os.path.join(data_dir, 'train-labels-idx1-ubyte.gz')
        # test_images_path = os.path.join(data_dir, 't10k-images-idx3-ubyte.gz')
        # test_labels_path = os.path.join(data_dir, 't10k-labels-idx1-ubyte.gz')
